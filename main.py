...
with open(os.path.join(os.path.dirname(__file__), 'Template', 'Temp_app'), 'r') as f:
    tmp_app = f.read()
'''Считываем файл-шаблон для контроллера'''
with open(os.path.join(os.path.dirname(__file__), 'Template', 'Temp_TREI'), 'r') as f:
    tmp_trei = f.read()
'''Считываем файл-шаблон для global'''
with open(os.path.join(os.path.dirname(__file__), 'Template', 'Temp_global'), 'r') as f:
    tmp_global = f.read()
...
for i in all_CPU:
    ...
    sl_CPU_one = is_load_ai_ae(i, cells)

    if len(sl_CPU_one) != 0:
        tmp_line_ = is_create_objects_ai_ae(sl_CPU_one, tmp_object_AIAE, 'Types.AI.PLC_AI')

        with open('file_out_group.txt', 'w') as f:
    ...
    if len(sl_CPU_one) != 0:
        tmp_line_ = is_create_objects_ai_ae(sl_CPU_one, tmp_object_AIAE, 'Types.AE.PLC_AE')
    ...
    file_clear = open('file_out_group.txt', 'w')
    file_clear.close()

'''Формирование выходного файла для каждого контроллера'''
with open('file_plc.txt', 'r') as f:
    tmp_line_ = f.read().rstrip()
